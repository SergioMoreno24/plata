...
const AlumnisOrg = ({ tok }) => {
    ...
    const changeFilters = () => {
        setLoading(true);
        let data = {state : stateA, generation, pathway};
        BtOrgServices.getFilteredAlumnis(tok, data)
        .then(res => {
            setAlumnis(res.data.alumnis);
            setLoading(false);
        })
            ...
            :
                <div className='w-full mt-2'>
                    <div className='w-full grid grid-cols-1 lg:grid-cols-2 gap-2'>
                        ...
                        <div className='w-full'><Button1 disabled={false} text='Filtrar' foo={ changeFilters }/></div>
                    </div>
