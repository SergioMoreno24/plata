...
const Rowprofile = ({ title, value }) => {
    return <div className='w-full py-2 border-b border-gray-300 grid grid-cols-1 lg:grid-cols-5 items-center text-sm my-1 border-b border-gray-300'>
        <div className='col-span-1 px-3 font-semibold text-blue-exm2'>{ title }</div>
        <div className='col-span-1 lg:col-span-4 px-3'>{ value }</div>
    </div>;
};

const Vacant = ({vacant, docs}) => {
    // console.log(vacant.reqdocs.substring(1, vacant.reqdocs.length -1 ));
    // console.log(docs);
    return (
        vacant.pdf ?<div className='w-full'>
            <div className='text-xl font-semibold text-blue-exm2 text-center my-3'>{vacant.charge}</div>
                ...
                <iframe className='w-full h-screen p-2' src={ `${process.env.REACT_APP_BASE_URL}/media/bt/vacants/${vacant.pdf}` } title={vacant.charge}></iframe>
            </div>
        :
